...
TEST_SIZE = 0.2
# features to use
FEATURE_COLUMNS = ["adjclose", "volume", "open", "high",
                   "low", "sma50", "sma200", "+bol20", "-bol20"]
# date now
...
def test_model(ticker, n_steps, lookup_step, test_size, feature_columns, loss, units, cell, n_layers, dropout, optimizer, bidirectional):
    ...
    mean_absolute_error = data["column_scaler"]["adjclose"].inverse_transform(
        mae.reshape(1, -1))[0][0]
    ...
    logData(date_now, ticker, mean_absolute_error, future_price,
            data['actual'], EPOCHS, lookup_step, accuracy, data['indicators'])
...
for ticker in tickers:
    train_model(ticker, N_STEPS, LOOKUP_STEP, TEST_SIZE, FEATURE_COLUMNS,
                LOSS, UNITS, CELL, N_LAYERS, DROPOUT, OPTIMIZER, BIDIRECTIONAL)
    test_model(ticker, N_STEPS, LOOKUP_STEP, TEST_SIZE, FEATURE_COLUMNS,
               LOSS, UNITS, CELL, N_LAYERS, DROPOUT, OPTIMIZER, BIDIRECTIONAL)
